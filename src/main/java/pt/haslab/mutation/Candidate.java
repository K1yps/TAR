...
public class Candidate {
    ...
    private static String calculateExtensionalityID(List<Mutator> mutators) {
        ...
        StringBuilder sb = new StringBuilder();
        List<Integer> hashCodes = mutators.stream().map(System::identityHashCode).sorted().toList();
    ...
    public String toJSON() {
        StringBuilder sb = new StringBuilder();
        sb.append("{");
        prunned.ifPresent(pruneReason -> sb.append("\"prunned\":\"").append(pruneReason).append("\","));
        sb.append("\"mutants\":[");
        if (mutators.size() > 0) {
        ...
        sb.append("\"children\":[");
        List<String> children = this.children == null ? new ArrayList<>() : this.children.stream().map(Candidate::toJSON).toList();
        if (children.size() > 0) {
